...
app.controller('progController', function ideController($scope, $sce, $http) {
    ...
    $http({
        ...
        if (data.files instanceof Array){
            console.log("File-data: found array adding to scope");
            $scope.files = data.files;
        } else {
            console.log("File-data: Array NOT found -> CONVERTING");
...
function json2array(json){
    ...
    keys.forEach(function(key){
        ...
        console.log("Convert: converting for key_"+key);
    });
    console.log("Array final: "+ result.toString());
    return result;
}