...
<ul class="breadcrumb">
	<?php for ( $i=1; $i<=$info[0]->stage;$i++ ) : ?>
		...
		<?php if ( $i <= 6) : ?>
			...
			<?php if ( $page != $i) : ?>
				<li><?php echo anchor("tc/stage$i/".$info[0]->class_id.'/'.$info[0]->session_id.'/'.$info[0]->prog_name , "Step $i") ?> <span class="divider">/</span> </li>
			<?php else: ?>
		...
		<?php else: if($i == 7) : ?>
				<?php if ( $page != $i) : ?>
				<li><?php echo anchor("tc/review/".$info[0]->class_id.'/'.$info[0]->session_id.'/'.$info[0]->prog_name , "Review") ?> </li>
				<?php else: ?>
