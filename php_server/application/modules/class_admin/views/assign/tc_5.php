...
for($i=1; $i<=$num_tc; $i++)
{
	...
	if ($info[0]->output == 1) // Check to see if cin is needed and provide space for that
	{
		...
		if ( isset($r_check) ) $r_val = $fileinfo[$r_check]->file_content; else $r_val = set_value("output_$i");  
	...
	{		 
		...
		{		
			if ( $file->multi_part == 1 && $file->stream_type != 'source' && $file->file_name != 'cout' && $file->meta == $i && $file->stream_type == "output")
			{
				...
				echo '<div class="controls">';
				if ( $file->file_content != "" ) $r_val = $file->file_content; else $r_val = set_value($file->id.'_'.$i);  
